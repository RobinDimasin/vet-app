...
export default function NewAppointmentForm({
  ...
  const createNewAppointment = useMutation(({ date, pets }) => {
    return makeApiPostRequest("/api/appointment/new", {
      date,
      pets,
    });
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setFieldError, resetForm }) => {
      ...
      if (account && account.account_type === "owner") {
        const response = await createNewAppointment.mutateAsync({
          date: values.date,
        ...
        submitButton={
          <button
            className={`btn btn-primary fluid ${
              createNewAppointment.isLoading ? "loading" : ""
            }`}
            type="submit"
          >
            Submit
        ...
        <FieldArray name="pets">
          {({ insert, remove, push }) => (
            <div>
              ...
              {pets.filter(
                (petInChoices) =>
                  !formik.values.pets.some(
                    (petInForm) => petInForm.pet_id === petInChoices.id
                  )
              ).length === 0 ? (
                <div
                  className={`alert ${
                    pets.length === 0 ? "alert-warning" : ""
                  }`}
                >
                  <div>
                    <Icon icon={<InfoIcon />} />
                    <span>
                      {pets.length === 0 ? "You have no pets" : "No pets left."}
                    </span>
                  </div>
                </div>
              ) : (
                <select
                  className={`select select-bordered select-sm w-full ${
                    formik.values.pets.length === 0 ? "select-primary" : ""
                  }`}
                  disabled={
                    pets.filter(
                      (petInChoices) =>
